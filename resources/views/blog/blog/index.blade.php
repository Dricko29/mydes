    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header border-bottom">
          ...
          <a href="{{ route('site.blog.blogs.create') }}" class="btn btn-primary">@lang('Add')</a>
        </div>
  ...
  <script>
    ...
    $(function () {
        ...
        if (dt_ajax_table.length) {
            ...
            processing: true,
            ajax: '{{ route('site.blog.blogs.index') }}', // JSON file to add data
            columns: [
          ...
          var id = [];
          {
              ...
              if(id.length > 0)
              {
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url:'{{ route('site.blog.blogs.bulkDelete') }}',
                            data:{id:id},
                            method:'post',
                            headers:{
    ...
    // single delete
    function hapus(e){
      var url = '{{ route("site.blog.blogs.destroy", ":id") }}';
