  ...
  <script>
    ...
    $(function () {
        ...
        if (dt_ajax_table.length) {
            ...
            processing: true,
            ajax: '{{ route('site.blog.categories.index') }}', // JSON file to add data
            columns: [
            ...
            columnDefs: [
              ...
              },
              {
                ...
                render: function (data, type, full, meta) {
                  ...
                  return (
                    '<a href="/site/blog/categories/' + full['id'] +'/show" class="item-edit me-1" title="detail">' +
                    feather.icons['list'].toSvg({ class: 'font-small-4' }) +
                    '</a>'+
                    '<a href="/site/blog/categories/' + full['id'] +'/edit" class="item-edit me-1" title="edit">' +
          ...
          var id = [];
          {
              ...
              if(id.length > 0)
              {
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url:'{{ route('site.blog.categories.bulkDelete') }}',
                            data:{id:id},
                            method:'post',
                            headers:{
    ...
    // single delete
    function hapus(e){
      var url = '{{ route("site.blog.categories.destroy", ":id") }}';
