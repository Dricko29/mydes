  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Statistik Status Penduduk</h4>
      </div>
        <div class="card-body border-bottom">
          <div class="row">
            <div class="col-md-4">
                <a href="{{ route('site.statistik.form.cetak', 'status') }}" class="btn btn-sm btn-primary">
        ...
        <table class="table table-bordered mb-0">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $status->sum('penduduk_count')}}</td>
              <td>{{ number_format(($status->sum('penduduk_count')/$pendudukTotal)*100),2}}%</td>
              <td>{{ $status->sum('laki')}}</td>
              <td>{{ number_format(($status->sum('laki')/$pendudukTotal)*100),2}}%</td>
              <td>{{ $status->sum('perempuan')}}</td>
              <td>{{ number_format(($status->sum('perempuan')/$pendudukTotal)*100),2}}%</td>
            </tr>
          </tbody>
        </table>
      </div>
