  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Statistik Agama Penduduk</h4>
      </div>
        <div class="card-body border-bottom">
          <div class="row">
            <div class="col-md-4">
                <a href="{{ route('site.statistik.form.cetak', 'agama') }}" class="btn btn-sm btn-primary">
        ...
        <table class="table table-bordered mb-0">
          <thead>
            <tr>
              ...
              <th scope="col" class="text-nowrap">Agama</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $agama->sum('penduduk_count')}}</td>
              <td>{{ number_format(($agama->sum('penduduk_count')/$pendudukTotal)*100),2}}%</td>
              <td>{{ $agama->sum('laki')}}</td>
              <td>{{ number_format(($agama->sum('laki')/$pendudukTotal)*100),2}}%</td>
              <td>{{ $agama->sum('perempuan')}}</td>
              <td>{{ number_format(($agama->sum('perempuan')/$pendudukTotal)*100),2}}%</td>
            </tr>
          </tbody>
        </table>
      </div>
