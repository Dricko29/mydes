  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Statistik Pekerjaan Penduduk</h4>
      </div>
        <div class="card-body border-bottom">
          <div class="row">
            <div class="col-md-4">
                <a href="{{ route('site.statistik.form.cetak', 'pekerjaan') }}" class="btn btn-sm btn-primary">
        ...
        <table class="table table-bordered mb-0">
          <thead>
            <tr>
              ...
              <th scope="col" class="text-nowrap">Pekerjaan</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $pekerjaan->sum('penduduk_count')}}</td>
              <td>{{ number_format(($pekerjaan->sum('penduduk_count')/$pendudukTotal)*100),2}}%</td>
              <td>{{ $pekerjaan->sum('laki')}}</td>
              <td>{{ number_format(($pekerjaan->sum('laki')/$pendudukTotal)*100),2}}%</td>
              <td>{{ $pekerjaan->sum('perempuan')}}</td>
              <td>{{ number_format(($pekerjaan->sum('perempuan')/$pendudukTotal)*100),2}}%</td>
            </tr>
          </tbody>
        </table>
      </div>
