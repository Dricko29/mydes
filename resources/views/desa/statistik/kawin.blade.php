...
@section('title', 'Statistik Status Kawin')
  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Statistik Status Kawin Penduduk</h4>
      </div>
        <div class="card-body border-bottom">
          <div class="row">
            <div class="col-md-4">
                <a href="{{ route('site.statistik.form.cetak', 'kawin') }}" class="btn btn-sm btn-primary">
        ...
        <table class="table table-bordered mb-0">
          <thead>
            <tr>
              ...
              <th scope="col" class="text-nowrap">Status Kawin</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $kawin->sum('penduduk_count')}}</td>
              <td>{{ number_format(($kawin->sum('penduduk_count')/$pendudukTotal)*100),2}}%</td>
              <td>{{ $kawin->sum('laki')}}</td>
              <td>{{ number_format(($kawin->sum('laki')/$pendudukTotal)*100),2}}%</td>
              <td>{{ $kawin->sum('perempuan')}}</td>
              <td>{{ number_format(($kawin->sum('perempuan')/$pendudukTotal)*100),2}}%</td>
            </tr>
          </tbody>
        </table>
      </div>
