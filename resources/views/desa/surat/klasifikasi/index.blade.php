    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header border-bottom">
          <h4 class="card-title">List Klasifikasi Surat</h4>
          <a href="{{ route('site.klasifikasiSurat.create') }}" class="btn btn-primary">@lang('Add')</a>
        </div>
  ...
  <script>
    ...
    $(function () {
        ...
        if (dt_ajax_table.length) {
            ...
            processing: true,
            ajax: '{{ route('site.klasifikasiSurat.index') }}', // JSON file to add data
            columns: [
            ...
            columnDefs: [
              ...
              },
              {
                ...
                render: function (data, type, full, meta) {
                  ...
                  return (
                    '<a href="/site/klasifikasiSurat/' + full['id'] +'/edit" class="item-edit me-1" title="edit">' +
                    feather.icons['edit'].toSvg({ class: 'font-small-4' }) +
                    '</a>'+
                    '<a href="/site/klasifikasiSurat/' + full['id'] +'/status" class="item-edit me-1" title="'+info+'">' +
                    icon +
                    '</a>'+
          ...
          var id = [];
          {
              ...
              if(id.length > 0)
              {
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url:'{{ route('site.kalsifikasiSurat.bulkDelete') }}',
                            data:{id:id},
                            method:'post',
                            headers:{
    ...
    // single delete
    function hapus(e){
      var url = '{{ route("site.klasifikasiSurat.destroy", ":id") }}';
