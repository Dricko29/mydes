...
class SuratUserController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $penduduk_id = LayananMandiri::where('user_id', auth()->user()->id)->first()->penduduk_id;
            $model = PermohonanSurat::with(['penduduk', 'surat'])->where('penduduk_id', $penduduk_id)->when($status, function ($query) use ($status) {
            ...
            return DataTables::eloquent($model)
                ...
                ->addColumn('info_status', function ($model) {
                    if ($model->status == 1) {
                        return '<a href="#"><span class="badge bg-info">Sedang Diperiksa</span></a>';
                    } elseif ($model->status == 2) {
                        return '<a href="#"><span class="badge bg-warning">Menunggu Ditandatangani</span></a>';
                    } elseif ($model->status == 3){
                        return '<a href="#"><span class="badge bg-primary">Siap Diambil</span></a>';  
                    } elseif ($model->status == 4){
                        return '<a href="#"><span class="badge bg-success">Sudah Diambil</span></a>';   
                    } elseif ($model->status == 0) {
                    return '<a href="#"><span class="badge bg-danger" title="'.$model->pesan.'">Ditolak</span></a>';
                }
                })
        ...
        return view('user.permohonan-surat', compact('pageConfigs'));
    }
}